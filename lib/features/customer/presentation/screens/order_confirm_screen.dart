  ...
  Future<void> _confirmOrder() async {
    ...
    try {
      ...
      final order = await service.createOrder(
        ...
        courierCode: isDelivery ? 'jne' : null,
        courierService: isDelivery ? 'reg' : null,
      );
      ...
      if (widget.prescriptionFile != null) {
        try {
          final bytes = await widget.prescriptionFile!.readAsBytes();
          await service.uploadPrescription(
            order.id,
            bytes,
            widget.prescriptionFile!.name,
          );
        } catch (_) {
          // Upload resep gagal, tapi order tetap dibuat
        }
      }
