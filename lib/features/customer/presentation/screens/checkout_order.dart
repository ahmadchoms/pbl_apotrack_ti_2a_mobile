...
class _CheckoutScreenState extends ConsumerState<CheckoutScreen> {
            ...
            children: [
              ...
              _sheetOption(
                ...
                () async {
                  ...
                  if (xfile != null) {
                    setState(() => _prescriptionFile = xfile);
                  }
                },
              ),
  ...
  Widget build(BuildContext context) {
    ref.listen(customerProfileProvider, (prev, next) {
      if (!next.isLoading && next.addresses.isNotEmpty) {
        final converted =
            ...
            child: Row(
              children: [
                if (_prescriptionFile != null) ...[
                  _PrescriptionThumbnail(file: _prescriptionFile!),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Text(
                      _prescriptionFile!.name,
                      style: const TextStyle(
                        fontSize: 12,
                        fontWeight: FontWeight.w600,
                        color: AppColors.textDark,
...
class _PrescriptionThumbnail extends StatelessWidget {
  const _PrescriptionThumbnail({required this.file});
  final XFile file;
      ...
      child: SizedBox(
        ...
        child: FutureBuilder<Uint8List>(
          ...
          builder: (context, snapshot) {
            if (!snapshot.hasData) {
              return const Icon(Icons.image, size: 24, color: AppColors.textLight);
            }
            return Image.memory(
              snapshot.data!,
              width: 44,
